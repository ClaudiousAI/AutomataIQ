...
- ✅ **Wave 1 — M02 Authentication & Authorization** **complete**. `BearerAuthMiddleware` (the only place `Authorization` is parsed) wires a local `JwtVerifier` (RS256, JWKS, RFC 7519) — production uses the same interface to plug into Keycloak's JWKS endpoint. `require_auth` + `require_role(Role)` FastAPI dependencies replace ad-hoc RBAC checks. `TenantContext` + `resolve_tenant_for_path` enforces FR-057 (path-vs-token reconciliation; `platform_admin` is the only role that crosses tenant boundaries). `AuthAuditLogger` Protocol + `InMemoryAuditLogger` (PostgreSQL DDL shipped for M03 to take over) writes an audit row for every auth outcome; failures never crash the request (NFR-007). `/api/v1/me` + `/api/v1/auth/logout` are the user-facing surfaces. 7×7 RBAC matrix verified, 84.39% coverage, 94 tests pass, ruff + mypy strict both clean. **CI gate repair (post-merge)**: pytest import discovery (`pythonpath = ["."]` in `backend/pyproject.toml`), Conventional Commit workflow permissions (`pull-requests: read` + `checks: write`), and the latent PR-title gate `$GITHUB_OUTPUT` bug. All three landed in `a5ef32b`. **Deferred to M16**: token denylist (the `/auth/logout` audit-row no-op gets replaced by the Keycloak introspector's `jti` denylist when M16's deploy integration tests land). Traceability: FR-053, FR-054, FR-057, NFR-004, NFR-005, NFR-006, NFR-007.
- ✅ **Wave 1 — M03a Database & Storage — Schema + RLS Substrate** **complete**. The Postgres substrate (18-table schema, RLS policies on every tenant-scoped table, `pg_trgm` + `pgcrypto` extensions, three roles with grants, `app/db/tenant.py` as the single source of truth for the `app.tenant_id` GUC, idempotent Alembic migration `0001_initial_schema.py`) landed via PR #4 (`feat/M03a-schema-rls` → `main`). NFR-007 recoverability proven via `SET LOCAL` transaction-scoped semantics; `audit_log` NULL-actor carve-out documented in ADR-0018. Traceability: FR-001, FR-008, FR-019, FR-038, FR-043, FR-057, NFR-004, NFR-007. **See [`docs/29_Audit_Report_Pre_M03a_Close.md`](./29_Audit_Report_Pre_M03a_Close.md) for the pre-merge audit and [`docs/28_M03a_Design.md`](./28_M03a_Design.md) for the design.**
- ⏳ Next: **M03 — Database & Storage** (PostgreSQL schema, Row-Level Security scaffolding, query-builder conventions, RLS-friendly tenant scoping). Depends on M02 (the `tenant_id` claim is the RLS session variable). **M04 (Core Backend)** can also begin now per the docs/22 §3 wave ordering — it depends on M01+M02+M03, so it'll be unblocked as soon as M03 lands.
