...
- [ ] **AC-3c** Idempotent seed script `backend/app/db/neo4j/seed.cypher` (or `seed.py` if a programmatic generator is preferred): re-running produces no duplicates, no orphaned nodes. Seed includes: the **16 SAP domain enums from FR-019** (FI/CO, SD, MM, PP, QM, PM/EAM, EWM, TM, PS, PLM, MDG, GRC, Treasury, CRM, HCM, procurement — per `docs/02 §3.3`); the **3 evidence-label enums from FR-013** (`confirmed | inferred | speculative` — per ADR-0007); and the **7 change-classification enums from FR-010** (new capability, enhancement, documentation clarification, deprecation, architecture change, event announcement, no meaningful change — per `docs/02 §3.2`). The 4th UI-only label `corroborated` (per `docs/11 §113`) is a frontend badge rendering, not a Neo4j property value, and is therefore out of scope for the seed.
...
- [ ] **AC-3e** `backend/app/storage/neo4j_adapter.py` ships the `Neo4jAdapter` **implementation**, importing the Protocol from [`006`](./006-m03b-storage-adapter-interfaces.md) (`from backend.app.storage.protocols import Neo4jAdapter`). The implementation must satisfy every method on the Protocol; the Protocol's `WITH {tenant_id: $tenant_id} AS ctx REQUIRE ctx.tenant_id IS NOT NULL` prepend pattern on `cross_domain_query` / `temporal_query`, the FR-041 evidence + confidence enforcement via `merge_node` / `merge_relationship`, and the `tenant_id`-first invariant are the source of truth and live in `006`. This sub-issue ships only the implementation — the contract is locked upstream so it cannot drift across the four adapters.
