...
- [ ] **AC-4d** `backend/app/storage/minio_adapter.py` ships `MinioAdapter` Protocol with: `put_object` (with versioning-aware logic), `get_object` (latest or specific version_id), `delete_object`, `presigned_url`, `bucket_health`, `lifecycle_status`. Inherits the shape pattern from `002`'s `RedisAdapter`.
