...
- [ ] **AC-1c** `backend/app/storage/redis_adapter.py` ships `RedisAdapter` Protocol with the operations M04 will call: `publish`, `consume`, `ack`, `dlq_publish`, `reconnect`, `stream_health`. Docstrings cross-link to the M04 event-bus runtime helper that will wrap them.
