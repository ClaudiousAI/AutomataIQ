...
- [ ] **AC-2d** `backend/app/storage/qdrant_adapter.py` ships `QdrantAdapter` Protocol with the operations M04 will call: `upsert`, `search`, `delete_by_tenant`, `collection_health`, `payload_filter_builder`. Inherits the shape pattern from `002`'s `RedisAdapter` (per `docs/25 §11` consistency rule).
